...
#   ./setup_conda_onCar.sh   # creates the 'thirdimpact' env, installs deps, builds everything
#
...
# registers the 'thirdimpact' alias into BOTH ~/.bashrc and ~/.zshrc.
#
# What it does, in order (see the README "Get started" section for the why):
#   1) conda env create -f environment.yml          -> env 'thirdimpact' (ROS 2 Jazzy)
#   2) register `alias thirdimpact='source .../thirdimpact.sh'` in ~/.bashrc + ~/.zshrc
...
REPO="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"   # .../src/racing_stack
WS="$(cd "$REPO/../.." && pwd)"                            # colcon workspace root
ENV_NAME="thirdimpact"
...
# --- 2. register the 'thirdimpact' alias in bash AND zsh ----------------------
ALIAS_LINE="alias thirdimpact='source $REPO/thirdimpact.sh'"
for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
  ...
  touch "$rc"
  if grep -qF "source $REPO/thirdimpact.sh" "$rc"; then
    say "alias already in $(basename "$rc") — leaving it"
  else
    printf '\n# racing_stack: enter the dev env with `thirdimpact`\n%s\n' "$ALIAS_LINE" >> "$rc"
    say "added 'thirdimpact' alias to $(basename "$rc")"
  fi
done
...
# into install/setup.bash; then every `thirdimpact` entry re-sources ROS1 and nodes die
...
say "done. Open a NEW shell (or 'source ~/.bashrc') then run:  thirdimpact"
